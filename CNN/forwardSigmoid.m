function [net, output, mapsize] = forwardSigmoid(net, input)
net{1}.x = input;
numIn = length(input);
mapsize = [32,32];  
for i = 1:length(net)
    switch net{i}.type
        case 'conv'
            ...
            for k = 1:net{i}.numOut
                ...
                net{i+1}.x{k} = sigmoid(temp + net{i}.b{k});
            end
            numIn = net{i}.numOut;
        case 'pooling'  %average pooling
            ...
            for j = 1:numIn
                ...
                net{i+1}.x{j} = sigmoid(temp(1:net{i}.scale:end, 1:net{i}.scale:end));
            end
        case 'full'
            fin = prod(mapsize) * length(net{i}.x);
            net{i}.tx = reshape(cell2mat(net{i}.x),1,fin);
            net{i+1}.x = sigmoid(net{i}.tx * net{i}.w + net{i}.b);
        case 'output'
            output = sigmoid(net{i}.x * net{i}.w + net{i}.b);
        otherwise
            error('!');
    end
end