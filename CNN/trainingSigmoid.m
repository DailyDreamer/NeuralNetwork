function net = trainingSigmoid(net, learnRate, epochNum)
...
for epoch = 1:epochNum
    ...
    for bat = 1:length(trainFile)-1
        ...
        for index = 1:length(labels)
            temp = data(index, :);
            input{1} = double(reshape(temp(1:1024), [32,32])')/255;
            input{2} = double(reshape(temp(1025:2048), [32,32])')/255;
            input{3} = double(reshape(temp(2049:3072), [32,32])')/255;
            %forward
            [net, output, mapsize] = forwardSigmoid(net, input);
            %backward
            %init T
            T = zeros(1,10);
            T(labels(index)+1) = 1;
            currentError = 0.5 * sum((T-output).^2);
            outdelta = (output - T) .* output .* (1 - output);
            for i = length(net):-1:1
                switch net{i}.type
                    case 'conv'
                        for j = 1:length(net{i}.x)
                            ...
                            for k = 1:net{i}.numOut
                                net{i}.delta{j} = net{i}.delta{j} + conv2(net{i+1}.delta{k}, net{i}.w{j,k}, 'full') .* (net{i}.x{j} .* (1-net{i}.x{j}));
                    ...
                    case 'pooling'
                        for j = 1:length(net{i}.x)
                            net{i}.delta{j} = net{i}.x{j} .* (1 - net{i}.x{j}) .* kron(net{i+1}.delta{j}, ones(net{i}.scale)) / net{i}.scale^2;
                        end
                    case 'full'
                        net{i}.tdelta = net{i+1}.delta * net{i}.w' .* net{i}.tx .* (1 - net{i}.tx);   % [1,fin] =  [1,fout] * [fout,fin] .* [1,fin]
                    ...
                    case 'output'
                        net{i}.delta = outdelta * net{i}.w' .* net{i}.x .* (1 - net{i}.x);
