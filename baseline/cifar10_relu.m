...
conv1 = convLayer(5, 5, 3, 32, 0.01, 'relu', 0.002, 0.0005, 0.1); % 5 and 5 are filter size, 3 is input feature number, 32 is output feature number, 'relu' is activation type, 0.0001 is learning rate.
...
conv2 = convLayer(5, 5, 32, 32, 0.01, 'relu', 0.002, 0.0005, 0.1);
pool2 = poolLayer('avg', 3, 2, [0, 1, 0, 1]);
conv3 = convLayer(5, 5, 32, 64, 0.01, 'relu', 0.002, 0.0005, 0.1);
pool3 = poolLayer('avg', 3, 2, [0, 1, 0, 1]);
fc = fcLayer(1024, 4*4*64, 0.01, 'relu', 0.001, 0.0005, 0.1); % 10 is output feature number, 64 is input feature number
fc1 = fcLayer(512, 1024, 0.01, 'relu', 0.001, 0.0005, 0.1);
fc2 = fcLayer(10, 512, 0.01, 'none', 0.001, 0.0005, 0.1);
...
for epoch = 1:N_epoch
    ...
    for batch = 1:N_batch
        ...
        if mod(batch, 4) == 0
            ...
            softmax = backward(softmax, label);
            fprintf(['       Valid batch ' num2str(batch / 4) ', loss is ' num2str(softmax.loss) ', accuracy is ' num2str(softmax.accuracy) ', elapsed time is ' num2str(toc) ' seconds.\n']);
            tic;
        end
    end
end
